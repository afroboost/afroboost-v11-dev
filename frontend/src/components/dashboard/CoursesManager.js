...
const CoursesManager = ({
  ...
  return (
    <div className="card-gradient rounded-xl p-6">
      <h2 className="font-semibold text-white mb-4" style={{ fontSize: '20px' }}>{t('courses')}</h2>
      ...
      <div className="glass rounded-lg p-3 mb-4" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div
          className={`switch ${autoReviewEnabled ? 'active' : ''}`}
      ...
      <div style={{ maxHeight: '600px', overflowY: 'auto', paddingRight: '8px' }} className="custom-scrollbar">
        {courses.filter(c => !c.archived).map((course, idx) => (
          <div key={course.id} className="glass rounded-lg p-4 mb-4 relative">
            ...
            <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid rgba(139,92,246,0.15)', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              <button
                onClick={() => sendReviewRequest(course)}
