/**
 * ContactsManager.js — v18: Gestion centralisée des contacts
...
export default function ContactsManager({ API, coachEmail }) {
  ...
  const [filterType, setFilterType] = useState('all'); // all, user, group, google
  ...
  return (
    <div style={{ padding: '0' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: syncing ? '10px' : '0' }}>
          <div>
        ...
        {syncing && (
          <div>
            <div style={{
              ...
              <div style={{
                width: `${syncProgress}%`, height: '100%', borderRadius: '3px',
                background: 'linear-gradient(90deg, #22c55e, #34d399)',
                transition: 'width 0.5s ease'
              }} />
            </div>
            {syncMessage && (
              <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.6)', margin: '6px 0 0 0' }}>{syncMessage}</p>
            )}
          </div>
        )}
      </div>
      ...
      {importResult && (
        <div style={{
          ...
          background: importResult.imported > 0 ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)',
          border: `1px solid ${importResult.imported > 0 ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}`
        }}>
      ...
      {/* Contacts List — virtualized with max height */}
      <div style={{
            ...
            <div key={c.id || i} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {c.type === 'group' ? `${c.member_count || 0} membres` : [c.email, c.phone].filter(Boolean).join(' • ') || 'Pas de coordonnées'}
                </div>
              </div>
              {c.tags?.length > 0 && (
