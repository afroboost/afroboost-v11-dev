...
const PaymentConfigTab = ({ paymentConfig, setPaymentConfig, coachEmail }) => {
  ...
  const toggleStyle = (enabled) => ({
    ...
    height: '24px',
    borderRadius: '12px',
    background: enabled ? 'var(--primary-color, #D91CD2)' : 'rgba(255,255,255,0.15)',
    cursor: 'pointer',
    position: 'relative',
                ...
                {['sandbox', 'live'].map(mode => (
                  <button key={mode} onClick={() => handleChange('paypal_mode', mode)}
                    style={{
                      padding: '8px 20px',
                      borderRadius: '8px',
                      border: `1px solid ${paymentConfig?.paypal_mode === mode ? 'var(--primary-color, #D91CD2)' : 'rgba(255,255,255,0.15)'}`,
                      background: paymentConfig?.paypal_mode === mode ? 'rgba(var(--primary-rgb, 217, 28, 210), 0.15)' : 'rgba(255,255,255,0.03)',
                      color: paymentConfig?.paypal_mode === mode ? 'var(--primary-color, #D91CD2)' : 'rgba(255,255,255,0.5)',
                      cursor: 'pointer',
                      fontSize: '13px',
                      fontWeight: '500',
