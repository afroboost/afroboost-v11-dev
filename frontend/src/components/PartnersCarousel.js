...
const PartnerVideoCard = ({ partner, onToggleMute, isMuted, onLike, isLiked, onNavigate, isPaused, onTogglePause, isVisible, maintenanceMode = false, isSuperAdmin = false, onBuyVideo, socialCommentsCount = 0, onShowComments, pageLikesCount = 0, likeAnimating = false }) => {
  ...
  useEffect(function() {
    ...
    for (var i = 0; i < heroVideosSorted.length; i++) {
      ...
      if (mediaType === 'image') {
        var imgUrl = resolveOptimizedImageUrl(heroVideosSorted[i].url || '') || resolveHeroUrl(heroVideosSorted[i].url || '');
    ...
    for (var j = 0; j < imageUrls.length; j++) {
      imagePromises.push(new Promise(function(resolve) {
        var img = new Image();
        img.onload = function() { onItemLoaded(); resolve(true); };
        img.onerror = function() { onItemLoaded(); resolve(false); };
        img.src = imageUrls[j];
      }));
    }

    // V153+V149.1: Video chunk loading with progress
    var videoPromise;
    if (videoSlideUrl) {
      var fileIdMatch = videoSlideUrl.match(/\/api\/files\/([a-zA-Z0-9]+)\//);
      if (!fileIdMatch) {
        // No valid file ID — count video placeholder as loaded
        onItemLoaded();
        videoPromise = Promise.resolve();
      } else {
        var fileId = fileIdMatch[1];
        videoPromise = fetch(base + '/api/video-chunks/' + fileId + '/info')
          .then(function(r) { return r.json(); })
          .then(function(info) {
            if (canceled) return;
            console.log('[V153] Video: ' + info.file_size + ' bytes, ' + info.total_chunks + ' chunks');
            // V149.1: Replace the 1 video placeholder with actual chunk count
            totalItems = imageUrls.length + info.total_chunks;
            updateProgress();
            ...
            for (var c = 0; c < info.total_chunks; c++) {
              chunkPromises.push(
                fetch(base + '/api/video-chunks/' + fileId + '/' + c)
                  ...
                  .then(function(buf) { onItemLoaded(); return buf; })
                  .catch(function(err) { onItemLoaded(); return null; })
              );
            }
          ...
          .catch(function(err) {
            ...
            // Count video as loaded even on error so splash dismisses
            onItemLoaded();
          });
      }
    } else {
    ...
    // V149.1: Wait for ALL media (images + video) then dismiss splash
  ...
  // V148: Poster image for video slides — use first image from heroVideos as poster
  const videoPosterUrl = useMemo(function() {
    if (currentHeroType !== 'video') return '';
    // Find first image in heroVideos to use as poster
    for (var i = 0; i < heroVideosSorted.length; i++) {
      var mediaType = detectHeroMediaType(heroVideosSorted[i]);
      if (mediaType === 'image') {
        return resolveOptimizedImageUrl(heroVideosSorted[i].url || '') || resolveHeroUrl(heroVideosSorted[i].url || '');
      }
    }
    return '';
  }, [currentHeroType, heroVideosSorted]);
                        ...
                        ref={function(el) {
                          if (el) {
                            ...
                            var tryPlay = function(attempt) {
                              if (attempt > 60) {
                                console.warn('[V153] Video failed after 60 attempts, staying on poster');
                                return;
                              }
                              setTimeout(function() {
                                if (el.paused && el.readyState >= 2) {
                                  ...
                                  el.play().then(function() {
                                    console.log('[V153] Video playing! attempt=' + attempt + (preloadedBlobRef.current ? ' (from blob)' : ' (from range)'));
                                    setVideoReady(true);
